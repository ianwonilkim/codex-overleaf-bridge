...
버전: 1.8.0
채택 범위: 2026-09-09 v6 간편 연결, 프로젝트별 자동 쓰기 준비와 선택적 논문 규칙
...
않는다. 별도 TEST 프로젝트와 학회·저널 논문 규칙 프로필은 원하는 사용자만 쓰는
선택 사항이다.
...
브리지를 일반 Overleaf 제어기로 쓰는 프로젝트는 규칙 프로필을 생략할 수 있으며,
이 때문에 연결 또는 쓰기를 차단하지 않는다. 규격 관리가 필요한 프로젝트에는
다음을 project ID별로 저장한다.

- venue/year/track 또는 팀 규칙명과 revision
- 검토 시각과 공식 HTTPS 출처
- 정확한 요구사항, 필수/권고 여부
- `source_diff`, `compile_pdf`, `human_final`, `live_official`, `advisory` 검사 분류

프로필은 연결할 때 함께 넣거나 이미 연결된 프로젝트에 나중에 추가·교체·삭제할
수 있다. 변경은 Overleaf 원문을 수정하지 않고 새 policy hash를 만들며, 과거 hash
기록을 보존하고 기존 미승인 diff를 무효화한다.

브리지는 template hash와 경로 같은 기술 조건만 직접 강제한다. 페이지 수는
compile/PDF, 저자·윤리·익명성은 사람의 최종 확인, deadline처럼 바뀔 수 있는 정책은
공식 출처 재확인이 필요하다. 규칙 프로필은 이 검사를 빠뜨리지 않게 하는
프로젝트별 계약이지 자동 제출 적합성 보증이 아니다.

## 4. 승인 의미

- `이 프로젝트 연결해`: Overleaf 문서를 바꾸지 않고 현재 프로젝트의 보호
  기준선을 등록·검증한다.
- `이 프로젝트에 ... 규칙을 저장해`: 이미 연결된 프로젝트의 선택적 논문 규칙
  프로필을 추가 또는 교체한다. 원문은 바꾸지 않는다.
- `이 프로젝트 규칙을 지워줘`: 논문 규칙만 제거하고 연결 안전장치는 유지한다.
