...
여러 작업을 함께 볼 때는 ChatGPT 데스크톱 앱의 Codex 모드가 가장 편하다. 논문마다
프로젝트 하나와 `논문 작성 메인` 채팅 하나를 만들고, 실험·문헌 조사·그림·검토는
별도 채팅으로 나눈다. 관련 채팅을 메인 Codex 채팅에 추가하거나 링크와 짧은 요약을
남겨 메인 채팅이 전체 논문을 조율하게 한다. Overleaf 연결과 실제 반영은 메인
채팅 하나에서 진행하는 것을 권장한다.

링크나 고정은 정리 수단이며 모든 대화 내용을 자동으로 합치지 않는다. 채택한
결과와 중요한 결정은 메인 채팅 또는 프로젝트 정책·기록 문서에도 짧게 남긴다.
서버 CLI에서도 같은 원칙을 쓸 수 있지만 여러 채팅을 한눈에 관리하기에는 앱이 더
편하다.
