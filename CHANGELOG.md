...
- Removed the independent `productionEnabled` flag and the single active
  production-project slot. A verified per-project policy is now the write gate,
  so multiple papers can remain connected without manual enable/disable steps.
- Added the user-facing `overleaf_connect_project` tool and concise one-diff
  confirmations while retaining legacy confirmation compatibility.
- Made the macOS installer project-agnostic and able to register the local MCP
  server automatically. Added a one-command lab-kit installer wrapper.
- Reduced the user workflow to install, open a project, connect it, preview a
  change, and approve that change. TEST remains optional.
